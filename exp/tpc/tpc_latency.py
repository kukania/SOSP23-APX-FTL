...
sheet_name = "16_frag_tpc"
output_path = "" # default: ""
output_name = "TPC_latency" 
#output_name = "TPCH"
...
result_path = "FAST21-RESULT-throughput.xlsx" #분석위한 xlsx파일 path 입력
#sheet_name = "16_fio"#분석할 sheet name 입력 
sheet_name = "16_frag_fio"
output_path = "" # default: ""
output_name = "fio_frag_latency" 
#output_name = "TPCH"
'''
...
for i in range(len(sub_list)):
    point = 70
    for j in range(6):
        tmp2 = sub_list[i].plot(x_list[j+6*i], y_list[j+6*i], linewidth=0.8,color=color_list[j], markerfacecolor="None",marker=shape_list[j], markersize=4.6, markevery=point)
        tmp3 = in_list[i].plot(x_list[j+6*i], y_list[j+6*i], linewidth=0.8,color=color_list[j], markerfacecolor="None",marker=shape_list[j], markersize=4.6, markevery=28)
...
fig.legend(tmp_list[:6], labels=["PAGE","COARSE","FINE","SFTL", "TPFTL", "APPX"], frameon=False,loc="upper center", bbox_to_anchor=(0.51, 1.07),ncol=6, fontsize=14)
